        ...
        $formattedData = $livraisons->map(function ($livraison) {
            return [
                ...
                'demande_livraison' => $livraison->demandeLivraison ? [
                    ...
                    'depose_au_depot' => $livraison->demandeLivraison->depose_au_depot,  // ✅ AJOUTER CETTE LIGNE pour le badge
...
public function update(Request $request, $id): JsonResponse
{
    ...
    $validator = Validator::make($request->all(), [
        ...
        // Colis fields - ⚠️ CORRECTION : accepter colis_poids, colis_type, colis_prix
    ...
    try {
        ...
        // 3. Mettre à jour le colis - ⚠️ CORRECTION : utiliser colis_poids, colis_type, colis_prix
 ...
 * ⭐ NOUVELLE MÉTHODE POUR LE GESTIONNAIRE
 */
public function store(Request $request): JsonResponse
{
    ...
    try {
        ...
        $initialStatus = $isDepotClient ? 'en_attente' : 'en_attente';

        // 5. Générer un code PIN unique
        $codePin = $this->generateUniquePin();

        // 6. Déterminer le gestionnaire connecté
        $user = Auth::user();
        $gestionnaire = $user->gestionnaire;
